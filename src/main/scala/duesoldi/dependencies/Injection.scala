...
object Injection
{
  ...
  def inject[IN1, IN2, OUT](fn: (IN1, IN2) => OUT)(implicit in1: Inject[IN1], in2: Inject[IN2], config: Config): Inject[OUT] = config => fn(in1(config), in2(config))

  def inject[IN1, IN2, IN3, OUT](fn: (IN1, IN2, IN3) => OUT)(implicit in1: Inject[IN1], in2: Inject[IN2], in3: Inject[IN3], config: Config): Inject[OUT] = config => fn(in1(config), in2(config), in3(config))

  def inject[IN1, IN2, IN3, IN4, OUT](fn: (IN1, IN2, IN3, IN4) => OUT)(implicit in1: Inject[IN1], in2: Inject[IN2], in3: Inject[IN3], in4: Inject[IN4], config: Config): Inject[OUT] = config => fn(in1(config), in2(config), in3(config), in4(config))
