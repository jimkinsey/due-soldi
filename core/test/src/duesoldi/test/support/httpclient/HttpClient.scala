...
object HttpClient
{
  ...
  def get(path: String, env: Env, headers: Seq[(String, String)] = Seq.empty)(implicit ec: ExecutionContext): Future[Response] = {
    Http.default(url(s"http://localhost:${env("PORT")}$path").setHeaders(headers.map { case (name, value) => name -> Seq(value) } toMap)).map { res =>
      Response(
  ...
  def put(path: String, env: Env, body: String, headers: Seq[(String, String)] = Seq.empty)(implicit ec: ExecutionContext): Future[Response] = {
    Http.default(url(s"http://localhost:${env("PORT")}$path").setMethod("PUT").setBody(body).setHeaders(headers.map { case (name, value) => name -> Seq(value) } toMap)).map { res =>
      Response(
        status = res.getStatusCode,
        headers = convertHeaders(res.getHeaders),
        body = res.getResponseBody
  ...
  def delete(path: String, env: Env, headers: Seq[(String, String)] = Seq.empty)(implicit ec: ExecutionContext): Future[Response] = {
    Http.default(url(s"http://localhost:${env("PORT")}$path").setMethod("DELETE").setHeaders(headers.map { case (name, value) => name -> Seq(value) } toMap)).map { res =>
      Response(
        status = res.getStatusCode,
        headers = convertHeaders(res.getHeaders),
        body = res.getResponseBody
